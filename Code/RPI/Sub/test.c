...
void aes_gcm_encrypt(void)
	...
	printf("IV:\n");
	BIO_dump_fp(stdout, gcm_iv , sizeof(gcm_iv));
...
int main(int argc, char **argv)
	...
	unsigned char *plaintext = "hello world hii";
	int plaintext_len = strlen(plaintext);
    unsigned char *aad = {"hi"};
 ...
 unsigned char ciphertext[1024]= {0x0}; 
 unsigned char tag[16]={0x0};
 int len = gcm_encrypt(plaintext,plaintext_len, aad, aad_len, key,iv,iv_len,ciphertext, tag);
 printf("------------------------------------------------\n"); 
 printf("plain text:\n"); 
 ...
 BIO_dump_fp(stdout, iv, iv_len);
   ...
   printf("aad:\n"); 
 BIO_dump_fp(stdout, aad, aad_len);
 ...
 printf(" len = %d\n", len); 
 BIO_dump_fp(stdout, ciphertext, len);
 
  printf("------------------------------------------------\n");
  unsigned char plain[1024] ={0x0}; 
  
	int res = gcm_decrypt( ciphertext, len, aad, aad_len,tag,key,iv, iv_len,plain) ; 
	if (res >0)
	{
		 printf("plain text:\n"); 
