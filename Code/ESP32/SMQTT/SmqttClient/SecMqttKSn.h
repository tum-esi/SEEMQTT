...
class SecMqtt: public PubSubClient {
  ...
  public:
    ...
    void ibe_extract(int ks_id, unsigned char *Didc);
    int ibe_encryption(int ks_id, unsigned char *plaintext, size_t plaintext_len , unsigned char *cipher, unsigned char *Uc);
    void ibe_decryption(unsigned char *ciphertext, size_t ciphertext_len, unsigned char *Uc, unsigned char *Didc, unsigned char *plaintext);
