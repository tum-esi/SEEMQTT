...
void SecMqtt::secmqtt_set_enc_mode(char *mode) {
    ...
    if (!strncmp(mode, "ibe", 3)) {
        ...
        element_from_bytes(ibe_param_pri.Km, km_t);
        
        Serial.println("generate ibe id:");

        for (int i = 0; i < KSN_NUM; i++) {
            unsigned char hash[HASH_LEN] = {0};
            sprintf(ksn_list[i].ibe_id, "keystore%d@tum.com", i+1);
            ksn_list[i].ibe_id_len = strlen(ksn_list[i].ibe_id);
            Serial.println(ksn_list[i].ibe_id);
            
            /* Initialize parameters */
            element_init_G1(ksn_list[i].Qid, ibe_param_pub.pairing);
            element_init_G1(ksn_list[i].U, ibe_param_pub.pairing);
            element_init_GT(ksn_list[i].gid, ibe_param_pub.pairing);
            element_init_Zr(ksn_list[i].r, ibe_param_pub.pairing);
        ...
        #ifdef IBE_SELF_TEST
        Serial.println("+++ IBE self test start +++");
        Serial.println(ksn_list[0].ibe_id);
        unsigned char Didc[ELEMENT_LEN];
        
        unsigned char testmsg[32] = {0};
        esp_fill_random(testmsg, 32);
...
void SecMqtt::ibe_extract(int ks_id, unsigned char *Didc) {

    #ifdef DBG_MSG
    Serial.println("ibe extract start ...");
    #endif
    unsigned char hash[HASH_LEN] = {0};
    element_t Qid, Did;
    ...
    mbedtls_md(mbedtls_md_info_from_type(MBEDTLS_MD_SHA256), (unsigned char *)ksn_list[ks_id].ibe_id, ksn_list[ks_id].ibe_id_len, hash);
    ...
    #ifdef DBG_MSG
    Serial.println("ibe extract end!");
    #endif
}
...
int SecMqtt::ibe_encryption(int ks_id, unsigned char *plaintext, \
    ...
    */
    element_random(ksn_list[ks_id].r);
    element_mul_zn(ksn_list[ks_id].U, ibe_param_pub.P, ksn_list[ks_id].r);
    element_pow_zn(ksn_list[ks_id].gid, ksn_list[ks_id].gid, ksn_list[ks_id].r);
    ...
    element_to_bytes(Uc, ksn_list[ks_id].U);
    int Uc_len = element_length_in_bytes(ksn_list[ks_id].U);
...
void SecMqtt::ibe_decryption(unsigned char *ciphertext, size_t ciphertext_len, \
                         unsigned char *Uc, unsigned char *Didc, unsigned char *plaintext) {

    #ifdef DBG_MSG
    Serial.println("ibe decryption start ...");
    #endif
    ...
    mbedtls_md(mbedtls_md_info_from_type(MBEDTLS_MD_SHA256), (const unsigned char *)gs, element_length_in_bytes(xt), hash);
    ...
    #ifdef DBG_MSG
    Serial.println("ibe decryption end!");
    #endif
}
